...
module MTL
  def self.parse(filename)
    ...
    last = Phong::Material.new(Vec3::ZERO, Vec3::ZERO, Vec3::ZERO, 0.0, ConstantTexture.new(Vec3::ONE))
    ...
    File.each_line(filename) do |line|
      ...
      when "Ka"
        ...
        last.k_a = Vec3.new(values[0], values[1], values[2])
      when "Kd"
        values = tokens[1, 3].map(&.to_f)
        last.k_d = Vec3.new(values[0], values[1], values[2])
      when "Ks"
        values = tokens[1, 3].map(&.to_f)
        last.k_s = Vec3.new(values[0], values[1], values[2])
      when "map_Kd"
        name = tokens[1].gsub("\\", "/")
...
mat = Lambertian.new(Vec3.new(0.0, 0.0, 1.0))
...
raytracer = Phong::Raytracer.new(width, height,
                                 hitables: node,
                                 lights: [Phong::Light.new(Vec3.new(0.0, 1000.0, 0.0), 1.0)],
                                 ambient: 0.05,
                                 camera: camera,
                                 samples: 5,
