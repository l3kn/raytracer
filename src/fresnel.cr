class Fresnel
  def self.fresnel_dielectric(cos_i : Float64, cos_t : Float64, eta_i : Float64, eta_t : Float64)
    # Fresnel equations for parallel and perpenducular polarized light
    r_parallel = ((eta_t * cos_i) - (eta_i * cos_t)) / ((eta_t * cos_i) + (eta_i * cos_t))
    r_perpenducular = ((eta_i * cos_i) - (eta_t * cos_t)) / ((eta_i * cos_i) + (eta_t * cos_t))

    (r_parallel * r_parallel + r_perpenducular * r_perpenducular) * 0.5
  end

  def self.fresnel_conductor(cos_i : Float64, eta : Float64, k : Float64)
    tmp_f = (eta * eta + k * k)
    tmp = tmp_f * cos_i * cos_i
    r_parallel2 = (tmp - (2.0 * eta * cos_i) + 1.0) / (tmp + (2.0 * eta * cos_i) + 1.0)
    r_perpenducular2 = (tmp_f - (2.0 * eta * cos_i) + cos_i * cos_i) / (tmp_f + (2.0 * eta * cos_i) + cos_i * cos_i)

    (r_parallel2 + r_perpenducular2) * 0.5
  end
